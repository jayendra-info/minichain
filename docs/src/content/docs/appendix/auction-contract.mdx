...
The **English Auction** contract (`contracts/auction.asm`) implements an ascending-price auction on the Minichain VM. Bidders compete by placing ever-higher bids; when the timer expires the highest bidder wins and the seller can withdraw the proceeds.

| Property | Value |
|---|---|
| Source | `contracts/auction.asm` |
...
minichain deploy --from alice --source contracts/auction.asm --gas-limit 500000
```
</TabItem>
<TabItem label="Bid">
```bash
