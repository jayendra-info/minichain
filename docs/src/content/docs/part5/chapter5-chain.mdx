...
In [Chapter 4](/minichain/part4/chapter4-assembler), we built an assembler that compiles human-readable assembly into bytecode. Now we have all the building blocks:

- **Core primitives** ([Chapter 1](/minichain/part1/chapter1-core)) — Transactions, blocks, addresses, signatures
- **Persistent storage** ([Chapter 2](/minichain/part2/chapter2-storage)) — State and block storage with sled
- **Virtual machine** ([Chapter 3](/minichain/part3/chapter3-vm)) — Executes contract bytecode
- **Assembler** ([Chapter 4](/minichain/part4/chapter4-assembler)) — Compiles assembly to bytecode
...
<Aside type="tip">
**Looking for advanced topics?** After completing this chapter, see [Appendix C: Advanced Blockchain Concepts](/minichain/appendix/advanced-blockchain) for block reorganization, transaction receipts, and light clients.
</Aside>
...
```mermaid
graph TD
    Txn[User submits transaction] --> Mempool["Mempool (queue)"]
    Mempool["Mempool (queue)"] --> Validator["Validator (check rules)"]
    Validator["Validator (check rules)"] --> BlockProducer["Block Producer (authority creates block)"]
    BlockProducer["Block Producer (authority creates block)"] --> Executor["Executor (run transactions in VM)"]
    Executor["Executor (run transactions in VM)"] --> Storage["Storage (persist new state)"]
    Storage["Storage (persist new state)"] --> Block["New block added to chain"]
```
        ↓
   {/* 
        ↓
...
This is achieved through the storage layer's **batch operations** ([Chapter 2](/minichain/part2/chapter2-storage#batch-updates-with-atomic-commits)):
...
This chapter covered the core blockchain implementation. For production-grade features that extend beyond Minichain's scope, see [Appendix C: Advanced Blockchain Concepts](/minichain/appendix/advanced-blockchain), which covers:
