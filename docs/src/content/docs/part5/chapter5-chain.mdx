...
In PoA with a single authority, reorganizations (reorgs) don't occur — blocks are final. But in multi-authority PoA or PoS:

```
Normal chain:      A → B → C → D
Reorg occurs:      A → B → C' → D'
                       ↑ Fork point
```

Handling reorgs requires:
1. Detecting the fork point
2. Reverting blocks C and D
3. Applying C' and D'
4. Updating mempool (re-add reverted transactions)

<Aside type="note">
**For learning:** We skip reorg handling. In production PoA with multiple authorities, you'd implement fork choice rules (longest chain, highest block number, etc.).
</Aside>

### Transaction Receipts

Receipts provide proof that a transaction was executed:
...
pub struct TransactionReceipt {
    ...
    pub gas_used: u64,
    pub success: bool,
    pub logs: Vec<Log>, // Events emitted during execution
}
```

Receipts are stored separately from blocks and indexed by transaction hash for fast lookup.

### Light Clients

A **light client** doesn't store the full blockchain. Instead, it:
1. Stores only block headers
2. Verifies state using Merkle proofs
3. Trusts PoA signatures

```
Light client workflow:
1. Download block header (300 bytes)
2. Verify authority signature
3. Request Merkle proof for specific account
4. Verify proof against state_root
5. Trust the account balance
```
