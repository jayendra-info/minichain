...
In [Chapter 2](/minichain/part2/chapter2-storage), we built persistent storage for accounts and blocks. Now we need something to actually *execute* smart contract code. That's the **virtual machine** — the computational engine at the heart of our blockchain.
...
Our VM uses a **50-instruction** set organized into 9 categories: control flow, arithmetic, bitwise, comparison, memory, storage, immediate, context, and debug operations.

<Aside type="note" title="Complete Instruction Set Reference">
For the full instruction set with syntax, gas costs, and examples, see [Appendix C: Complete Instruction Set Reference](/minichain/appendix/instruction-set).

This section focuses on the VM implementation. For assembly programming usage, see [Chapter 4, Section 4.2](/minichain/part4/chapter4-assembler).
</Aside>
...
**Why separate Memory and Storage opcodes?**
...
| **Use Case** | Temporary computation, buffers | Contract state, balances |

This mirrors real computers: fast volatile RAM for active work, slow persistent disk for data that must survive.
</Aside>
...
The VM connects to the storage layer from [Chapter 2](/minichain/part2/chapter2-storage) via callbacks:
...
We have a VM, but writing bytecode by hand is tedious. In [Chapter 4](/minichain/part4/chapter4-assembler), we'll build an **assembler** that compiles human-readable assembly into bytecode:
