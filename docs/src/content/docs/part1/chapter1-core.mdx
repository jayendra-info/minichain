...
<a href="https://github.com/BLAKE3-team/BLAKE3/" target="_blank" rel="noopener noreferrer">Official BLAKE3 Repository →</a>
...
- <a href="https://cgi.cse.unsw.edu.au/~cs4601/refs/papers/blake3.pdf" target="_blank" rel="noopener noreferrer">BLAKE3 Paper (PDF)</a> — The original cryptographic specification
- <a href="https://en.wikipedia.org/wiki/BLAKE_(hash_function)" target="_blank" rel="noopener noreferrer">BLAKE Hash Function (Wikipedia)</a> — History and context
- <a href="https://www.youtube.com/watch?v=nk4nefmguZk" target="_blank" rel="noopener noreferrer">BLAKE3 Deep Dive (Video)</a> — Excellent technical explanation
- <a href="https://eprint.iacr.org/2019/1492.pdf" target="_blank" rel="noopener noreferrer">BLAKE3 CryptAnalysis</a> — Cryptanalysis of various cryptographic Algorithms
</Aside>
...
Since Ed25519 can't recover public keys, Minichain blocks must include an explicit `author` field (see section on [`author` field](#author--block-producer) below). This is a small overhead (~20 bytes per block) in exchange for faster cryptography.
...
- **Crate:** [`ed25519-dalek`](https://github.com/dalek-cryptography/curve25519-dalek) version 2.1
...
**For hashing (Blake3):**
- **Crate:** [`blake3`](https://github.com/BLAKE3-team/BLAKE3) version 1.5
...
<Aside type="tip" title="Want to learn more about elliptic curves?">
...
**See [Appendix A: Elliptic Curve Taxonomy](/minichain/appendix/elliptic-curves)** for a comprehensive guide covering:
...
![Address Derivation](../../../assets/diagrams/address-derivation.svg)
...
> **Core analogy:**
> A nonce is like a cheque number — the bank will only process each cheque number once, so you can't reuse an old signed cheque to withdraw money again.
...
<Aside type="note" title="How state_root enforces nonce integrity">
The nonce is part of the `Account` struct, which means every account's nonce is included in the world state. The block's `state_root` (covered [later](#state_root--world-state-commitment-and-the-need-of-it)) is a cryptographic commitment to **all account data**, including nonces.
...
> **Core analogy:**
> Gas is like a **wire transfer fee** — you set a maximum budget (`gas_limit`) and a rate you're willing to pay (`gas_price`). The bank charges you only for the actual work done, up to your limit.
...
![Merkle Tree Structure](../../../assets/diagrams/merkle-tree.svg)

Change any transaction → the root changes. This is a **merkle tree** — we'll cover it in detail in [section 1.6](#16-merkle-trees).
...
graph LR
    subgraph Before
        A1[Alice: 100<br/>Bob: 50<br/>Contract X: ...]
    end
    
    subgraph After
        A2[Alice: 70<br/>Bob: 80<br/>Contract X: ...]
    end
...
<Aside type="tip" title="state_root includes replay protection">
...
The state_root in each block header cryptographically commits to these nonce values, making historical replay attacks provably invalid. See the [Nonce section](#nonce-preventing-replay-attacks) for how nonces prevent replays.
</Aside>
...
We'll explore the state trie implementation in [Chapter 2: Storage](/minichain/part2/chapter2-storage).
...
**Also: Ed25519 requires it.** Unlike secp256k1 (Ethereum), you can't recover the public key from an Ed25519 signature. So we need `author` to look up the correct public key for verification. (See [Why Ed25519 Over secp256k1?](#why-ed25519-over-secp256k1) for a detailed comparison.)

We'll decide between single vs rotating authorities when implementing the consensus layer in [Chapter 5](/minichain/part5/chapter5-chain).
...
**For minichain:** We keep them as placeholders for now. A truly minimal PoA chain could remove both — we may do so once we finalize the consensus design in [Chapter 5](/minichain/part5/chapter5-chain).
...
```mermaid
flowchart TD
    Start[Block interval<br/>12 seconds] --> Decision{What's in<br/>mempool?}
    Decision -->|1000 txs| Full[Produce block with<br/>top txs up to<br/>gas limit]
    Decision -->|5 txs| Some[Produce block with<br/>those 5 txs]
    Decision -->|empty| Empty[Produce empty block<br/>still valid!]
```
...
We'll implement the mempool in [Chapter 5](/minichain/part5/chapter5-chain) as part of the chain orchestration layer.
</Aside>
...
```mermaid
graph LR
    B0["Block 0<br/>Genesis<br/>prev_hash=0"] --> B1["Block 1<br/>prev_hash=hash(Block0)"]
    B1 --> B2["Block 2<br/>prev_hash=hash(Block1)"]
```
...
graph TB
    ...
    HT0 & HT1 --> BH1["H(H(T0)+H(T1))"]
    HT2 & HT3 --> BH2["H(H(T2)+H(T3))"]
    
    BH1 & BH2 --> Root["Merkle Root<br/>H(Branch0+Branch1)"]
```
...
graph LR
    subgraph Prover
        T1[T1 data] --> H1["H(T1)"]
        H0["H(T0)<br/>sibling"] --> Comb1["H(T0)+H(T1)"]
        H23["H(T2+T3)<br/>uncle"] --> Comb2["Comb1+H23"]
    end
    
    H1 --> Comb1
    Comb1 --> Comb2
    Comb2 --> Root["Merkle Root"]
    Root -->|match| OK["✓ Verified"]
```

<Aside type="tip">
**Why This Matters:**
A block with 1,000 transactions has a Merkle tree of depth 10.
To prove any transaction is included, you only need:
...
With our primitives in place, we're ready to build the **storage layer** in [Chapter 2](/minichain/part2/chapter2-storage), where we'll persist accounts, blocks, and contract state using sled.
