...
For comprehensive documentation including all instructions, syntax, effects, gas costs, and examples, see [Appendix B: Complete Instruction Set Reference](/minichain/appendix/instruction-set).
...
<Aside type="tip">
**For detailed instruction documentation** including all syntax variations, gas costs, usage examples, and gotchas, see [Appendix B: Complete Instruction Set Reference](/minichain/appendix/instruction-set).
</Aside>
...
<Aside type="note">
This is a simplified version showing the pattern. A full parser handles all opcode variants, but they all follow the same structure: match the token, extract operands, build the AST node.
</Aside>
...
```mermaid
graph LR
    Asm[Assembly Code<br/>counter.asm] -->|Lexer| Tokens[Token Stream<br/>[LoadI, R0, 10, ...]]
    Tokens -->|Parser| AST[AST<br/>Program {...}]
    AST -->|Pass 1| Sym[Symbol Table<br/>main: 0x0000]
    Sym -->|Pass 2| Byte[Bytecode<br/>[0x70, 0x00, ...]]
    Byte -->|Loader| Mem[VM Memory<br/>PC=0, Gas=limit]
    Mem -->|Executor| Result[Execution Result<br/>Success/Revert]
```
