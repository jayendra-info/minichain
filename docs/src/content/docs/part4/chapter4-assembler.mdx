...
In [Chapter 3](/part3/chapter3-vm), we built a register-based virtual machine that executes bytecode. But writing raw bytecode is tedious and error-prone. Imagine debugging this:
...
This section provides a comprehensive reference for all VM opcodes. Each instruction shows its syntax, effect on VM state, and gas cost.

### Control Flow Instructions

| Instruction | Syntax | Effect | Gas Cost |
|------------|--------|--------|----------|
| `HALT` | `HALT` | Stop execution successfully | 0 |
| `NOP` | `NOP` | Do nothing (no operation) | 0 |
| `JUMP` | `JUMP Rtarget` | Unconditional jump: `PC = R[target]` | 8 |
| `JUMPI` | `JUMPI Rcond, Rtarget` | Conditional jump: if `R[cond] != 0`, `PC = R[target]` | 8 |
| `CALL` | `CALL Rtarget` | Call subroutine: push return address to R14, jump to `R[target]` | 700 |
| `RET` | `RET` | Return from subroutine: jump to address in R14 | 8 |
| `REVERT` | `REVERT` | Abort execution with error | 0 |
...
| Instruction | Syntax | Effect | Gas Cost |
|------------|--------|--------|----------|
| `ADD` | `ADD Rdst, Rs1, Rs2` | `R[dst] = R[s1] + R[s2]` (wrapping) | 2 |
| `SUB` | `SUB Rdst, Rs1, Rs2` | `R[dst] = R[s1] - R[s2]` (wrapping) | 2 |
| `MUL` | `MUL Rdst, Rs1, Rs2` | `R[dst] = R[s1] * R[s2]` (wrapping) | 3 |
| `DIV` | `DIV Rdst, Rs1, Rs2` | `R[dst] = R[s1] / R[s2]` (unsigned, traps if Rs2 = 0) | 5 |
| `MOD` | `MOD Rdst, Rs1, Rs2` | `R[dst] = R[s1] % R[s2]` (unsigned modulo) | 5 |
| `ADDI` | `ADDI Rdst, Rsrc, imm` | `R[dst] = R[src] + imm` (add immediate) | 2 |
...
| Instruction | Syntax | Effect | Gas Cost |
|------------|--------|--------|----------|
| `AND` | `AND Rdst, Rs1, Rs2` | `R[dst] = R[s1] & R[s2]` (bitwise AND) | 2 |
| `OR` | `OR Rdst, Rs1, Rs2` | `R[dst] = R[s1] \| R[s2]` (bitwise OR) | 2 |
| `XOR` | `XOR Rdst, Rs1, Rs2` | `R[dst] = R[s1] ^ R[s2]` (bitwise XOR) | 2 |
| `NOT` | `NOT Rdst, Rsrc` | `R[dst] = ~R[src]` (bitwise NOT) | 2 |
| `SHL` | `SHL Rdst, Rs1, Rs2` | `R[dst] = R[s1] << R[s2]` (logical left shift) | 5 |
| `SHR` | `SHR Rdst, Rs1, Rs2` | `R[dst] = R[s1] >> R[s2]` (logical right shift) | 5 |

### Comparison Instructions

These instructions set the destination register to 1 (true) or 0 (false):

| Instruction | Syntax | Effect | Gas Cost |
|------------|--------|--------|----------|
| `EQ` | `EQ Rdst, Rs1, Rs2` | `R[dst] = (R[s1] == R[s2]) ? 1 : 0` | 3 |
| `NE` | `NE Rdst, Rs1, Rs2` | `R[dst] = (R[s1] != R[s2]) ? 1 : 0` | 3 |
| `LT` | `LT Rdst, Rs1, Rs2` | `R[dst] = (R[s1] < R[s2]) ? 1 : 0` (unsigned) | 3 |
| `GT` | `GT Rdst, Rs1, Rs2` | `R[dst] = (R[s1] > R[s2]) ? 1 : 0` (unsigned) | 3 |
| `LE` | `LE Rdst, Rs1, Rs2` | `R[dst] = (R[s1] <= R[s2]) ? 1 : 0` | 3 |
| `GE` | `GE Rdst, Rs1, Rs2` | `R[dst] = (R[s1] >= R[s2]) ? 1 : 0` | 3 |
| `ISZERO` | `ISZERO Rdst, Rsrc` | `R[dst] = (R[src] == 0) ? 1 : 0` | 3 |
...
### Memory Instructions (RAM Operations)

These instructions access temporary linear memory (like RAM on a computer). Memory is fast and cheap, but data is **lost when execution ends**.

| Instruction | Syntax | Effect | Gas Cost |
|------------|--------|--------|----------|
| `LOAD8` | `LOAD8 Rdst, Raddr` | `R[dst] = Memory[R[addr]]` (1 byte) | 3 |
| `LOAD64` | `LOAD64 Rdst, Raddr` | `R[dst] = Memory[R[addr]]` (8 bytes, little-endian) | 3 |
| `STORE8` | `STORE8 Raddr, Rsrc` | `Memory[R[addr]] = R[src]` (1 byte) | 3 |
| `STORE64` | `STORE64 Raddr, Rsrc` | `Memory[R[addr]] = R[src]` (8 bytes, little-endian) | 3 |
| `MSIZE` | `MSIZE Rdst` | `R[dst] = size of memory in bytes` | 2 |
| `MCOPY` | `MCOPY Rdst, Rsrc, Rlen` | `memcpy(R[dst], R[src], R[len])` | 3 + len |

### Storage Instructions (Disk Operations)

These instructions access persistent contract storage (like a hard drive). Storage is slow and expensive, but data **persists across transactions**.

| Instruction | Syntax | Effect | Gas Cost |
|------------|--------|--------|----------|
| `SLOAD` | `SLOAD Rdst, Rkey` | `R[dst] = Storage[R[key]]` (persistent, 32-byte slot) | 100 |
| `SSTORE` | `SSTORE Rkey, Rvalue` | `Storage[R[key]] = R[value]` (persistent, 32-byte slot) | 5,000 (reset) or 20,000 (set) |
...
| Instruction | Syntax | Effect | Gas Cost |
|------------|--------|--------|----------|
| `LOADI` | `LOADI Rdst, imm64` | `R[dst] = imm64` (load 64-bit constant) | 2 |
| `MOV` | `MOV Rdst, Rsrc` | `R[dst] = R[src]` (register copy) | 2 |

### Context Instructions

These instructions provide information about the execution context:

| Instruction | Syntax | Effect | Gas Cost |
|------------|--------|--------|----------|
| `CALLER` | `CALLER Rdst` | `R[dst] = caller's address` (as u64, truncated) | 2 |
| `CALLVALUE` | `CALLVALUE Rdst` | `R[dst] = value sent with this call` | 2 |
| `ADDRESS` | `ADDRESS Rdst` | `R[dst] = this contract's address` | 2 |
| `BLOCKNUMBER` | `BLOCKNUMBER Rdst` | `R[dst] = current block number` | 2 |
| `TIMESTAMP` | `TIMESTAMP Rdst` | `R[dst] = current block timestamp` | 2 |
| `GAS` | `GAS Rdst` | `R[dst] = remaining gas` | 2 |
...
| Instruction | Syntax | Effect | Gas Cost |
|------------|--------|--------|----------|
| `LOG` | `LOG Rsrc` | Log `R[src]` value (for debugging, appears in execution trace) | 2 |
...
Recall from [Chapter 3](/part3/chapter3-vm#32-instruction-encoding), we have three main formats:
...
The VM tracer (from [Chapter 3](/part3/chapter3-vm#37-execution-tracing)) is invaluable for debugging:
