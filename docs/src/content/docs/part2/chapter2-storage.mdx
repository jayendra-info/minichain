...
In [Chapter 1](/part1/chapter1-core), we built all our blockchain primitives in memory. But a blockchain that forgets everything when you restart it isn't very useful. In this chapter, we'll build the **storage layer** — the persistent backbone that remembers accounts, blocks, and contract state.
...
<Aside type="note">
SLOAD and SSTORE are the VM opcodes for storage access. We'll use these names in [Chapter 3](/part3/chapter3-vm) when implementing the virtual machine.
</Aside>
...
From [Chapter 1](/part1/chapter1-core#state_root--world-state-commitment-and-the-need-of-it):
...
With persistence in place, we're ready to build the **virtual machine** in [Chapter 3](/part3/chapter3-vm). The VM will:
