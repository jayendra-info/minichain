...
graph TB
    subgraph KV[Key-Value Store]
        AA[account:0xAA...<br/>balance:1000]
        BB[account:0xBB...<br/>balance:500]
        CC[account:0xCC...<br/>balance:250]
    end
    
    AA --> HAA["H(A_key+A_val)"]
    BB --> HAB["H(B_key+B_val)"]
    CC --> HAC["H(C_key+C_val)"]
    
    HAA & HAB --> Branch1["H(A+B combined)"]
    HAC --> Branch2["H(C) - placeholder"]
    
    Branch1 & Branch2 --> Root[State Root<br/>Merkle Root]
```
...
Accounts Change -> Hash Changes -> State Root Changes

```mermaid
graph LR
    subgraph Initial["Initial State<br/>"]
        IRoot[State Root: 0x7d3f...]
        IRoot --> IA["Alice<br/>bal=1000"]
        IRoot --> IB["Bob<br/>bal=500"]
    end
    
    subgraph Final["After Transfer<br/>"]
        FRoot["State Root: 0x9e2a...<br/>"]
        FRoot --> FA["Alice<br/>bal=900<br/>−100"]
        FRoot --> FB["Bob<br/>bal=600<br/>+100"]
    end
    
    Initial --"txn = 100"--> Final
```
